...
--//
--// คุณสมบัติหลัก:
--//   - สุ่มไข่อัตโนมัติ (Infinity Hatch) **แก้ไขการดีเลย์**
...
--//   - ปิดอนิเมชันและ UI ที่ไม่จำเป็นเพื่อประสิทธิภาพสูงสุด **ตรวจสอบแล้ว**
...
-- ==========================
-- 🚀 PERFORMANCE PATCHES (ส่วนของการปิดอนิเมชัน)
-- ==========================
-- ส่วนนี้ทำงานเพื่อซ่อน GUI ที่เกี่ยวข้องกับการฟักไข่ทั้งหมด
...
-- ส่วนนี้จะเข้าไปแก้ไขสคริปต์ของเกมโดยตรงเพื่อปิดฟังก์ชันเล่นอนิเมชัน
task.spawn(function()
    local success, err = pcall(function()
        ...
        if env and env.PlayEggAnimation then 
            env.PlayEggAnimation = function() return end -- แทนที่ฟังก์ชันเดิมด้วยฟังก์ชันเปล่า
...
task.spawn(function()
    while true do
        if hatchRunning then
            ...
            -- [[ ✨ EDIT ]] ย้าย task.wait มาไว้ข้างในนี้
            -- เพื่อให้แน่ใจว่าการดีเลย์จะเกิดขึ้น 'หลัง' จากการสุ่มแต่ละครั้ง
            task.wait(HATCH_DELAY) 
        else
            -- หากไม่ได้ทำงาน ให้รอสักครู่เพื่อลดการใช้ทรัพยากร
            task.wait(0.5) 
        end
    end
end)
